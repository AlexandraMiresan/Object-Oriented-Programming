...
GUIDesigner::GUIDesigner( Administrator &admin, User &user, QWidget *parent): admin(admin), user(user),
                                                                                                        QWidget(parent), ui(new Ui::GUIDesigner), model(model) {
    loadEvents();
    currentIndex = -1;
...
QString GUIDesigner::formatDate(std::chrono::year_month_day &ymd) {
...
void GUIDesigner::connectSignalsAndSlots() {
    ...
    QObject::connect(ui->nextButton, &QPushButton::clicked, this, &GUIDesigner::cycleThroughEvents);
   ...
   // QObject::connect(ui->removeButton, &QPushButton::clicked, this, &GUIDesigner::removeEventUser);
    ...
    QObject::connect(ui->undoButton, &QPushButton::clicked, this, &GUIDesigner::undo);
    QObject::connect(ui->redoButton, &QPushButton::clicked, this, &GUIDesigner::redo);
    auto* undoShortcut = new QShortcut(QKeySequence(Qt::CTRL + Qt::Key_Z), this);
    connect(undoShortcut, &QShortcut::activated, this, &GUIDesigner::undo);

    auto* redoShortcut = new QShortcut(QKeySequence(Qt::CTRL + Qt::Key_Y), this);
    connect(redoShortcut, &QShortcut::activated, this, &GUIDesigner::redo);
    QObject::connect(ui->tableView, &QTableView::clicked, this, &GUIDesigner::removeEventUser);
}
...
void GUIDesigner::cycleThroughEvents(){
       currentIndex++;
...
void GUIDesigner::removeEventUser(const QModelIndex &index) {
    if (!index.isValid())
        return;

    int row = index.row();
    int eventId = model->getEventIdAtRow(row);

    QMessageBox::StandardButton reply;
    reply = QMessageBox::question(this, "Remove Event",
